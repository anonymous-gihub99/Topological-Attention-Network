"""
Train TAN/Topoformer on ArXiv Dataset - Fixed for Single-Label Classification with DataParallel
Implements training, validation, and testing for:
1. Classification (28 categories)
...
class ArXivDataset(Dataset):
    ...
    def __init__(self, data_path: str, split: str, tokenizer, max_length: int = 512, task: str = 'classification'):
        ...
        # Load data
        data_file = Path(data_path) / f'arxiv_{split}.pt'
        ...
        if task == 'classification':
            ...
            # Load QA data
            qa_file = Path(data_path) / 'arxiv_qa.json'
            ...
            # Load retrieval data
            ret_file = Path(data_path) / 'arxiv_retrieval.json'
...
def main():
    ...
    # Configuration - Updated with requested changes
    config = {
        'data_dir': './processed_data',
        'save_dir': './arxiv_models',
        'batch_size': 32,  # Changed from 16 to 32
        'learning_rate': 2e-5,
        'weight_decay': 0.01,
        'num_epochs': 10,  # Changed from 20 to 10
        'max_length': 512,
        'arxiv_config': {
            'vocab_size': 50000,
            'embed_dim': 768,
    ...
    # Create model
    clf_config = TopoformerConfig(**config['arxiv_config'])
    clf_model = TopoformerForSingleLabelClassification(clf_config, num_labels=28)
    
    # Create datasets
    train_dataset = ArXivDataset(config['data_dir'], 'train', tokenizer, task='classification')
    val_dataset = ArXivDataset(config['data_dir'], 'val', tokenizer, task='classification')
    test_dataset = ArXivDataset(config['data_dir'], 'test', tokenizer, task='classification')
    ...
    if qa_model:
        # Create datasets
        train_dataset_qa = ArXivDataset(config['data_dir'], 'train', tokenizer, task='qa')
        val_dataset_qa = ArXivDataset(config['data_dir'], 'val', tokenizer, task='qa')
        test_dataset_qa = ArXivDataset(config['data_dir'], 'test', tokenizer, task='qa')
    ...
    if ret_model:
        # Create datasets
        train_dataset_ret = ArXivDataset(config['data_dir'], 'train', tokenizer, task='retrieval')
        val_dataset_ret = ArXivDataset(config['data_dir'], 'val', tokenizer, task='retrieval')
        test_dataset_ret = ArXivDataset(config['data_dir'], 'test', tokenizer, task='retrieval')
    ...
    # Save all results
    results_path = Path(config['save_dir']) / 'arxiv_tan_results.json'
...
def create_summary_plot(results: Dict, save_dir: Path):
    ...
    plt.suptitle('TAN/Topoformer Performance on ArXiv Dataset (Multi-GPU)', fontsize=16)
    plt.tight_layout()
    plt.savefig(save_dir / 'arxiv_summary_results.png', dpi=300, bbox_inches='tight')
    plt.close()
